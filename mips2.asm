# Programa en ensamblador MIPS para encontrar el número menor entre 3 y 5 números ingresados por el usuario
# Autor: Tu Nombre
# Fecha: Fecha Actual
...
output_message: .asciiz "El número menor es: "
...
start_loop:
    ...
    move $s1, $zero    # Inicializar $s1 a 0 (número menor)
...
input_loop:
    ...
    # Comparar el número ingresado con el número menor actual
    blt $t2, $s1, update_min   # Si $t2 < $s1, actualizar $s1 (número menor)
    addi $s0, $s0, 1   # Incrementar el contador de números ingresados
    blt $s0, $t0, input_loop  # Si $s0 < $t0, volver a input_loop

update_min:
    move $s1, $t2      # Actualizar $s1 con el nuevo número menor
...
exit:
    ...
    move $a0, $s1      # Cargar el número menor en $a0
    syscall            # Llamar al syscall para imprimir el número menor
